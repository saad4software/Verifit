...
interface JdsDashboardProps {
  initialJds: Jd[]
}

export function JdsDashboard({ initialJds }: JdsDashboardProps) {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      console.error('Failed to refresh JDs:', err)
    } finally {
  ...
  const filteredJds = jds.filter((j) => {
    ...
    const title = j.content?.fields?.find((f) => f.category === 'title')?.text?.toLowerCase() || ''
    const company = j.content?.fields?.find((f) => f.category === 'company')?.text?.toLowerCase() || ''
    const location = j.content?.fields?.find((f) => f.category === 'location')?.text?.toLowerCase() || ''
    const sourceText = j.source.text?.toLowerCase() || ''
    return (
  ...
  return (
    <div data-testid="jds-dashboard" className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            Your private library of opportunities, structured and ready for intelligent CV tailoring.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            type="button"
            data-testid="refresh-jds-button"
      ...
      {jds.length > 0 && (
        <div className="mb-6 relative">
          ...
          <input
            type="text"
            data-testid="search-jds-input"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search job descriptions by title, company, or requirement..."
      ...
      {jds.length === 0 ? (
        ...
        >
          ...
          <h3 className="mt-4 text-lg font-bold text-slate-900 dark:text-white">
            No job descriptions imported yet
          </h3>
          <p className="mt-1 max-w-sm text-xs text-slate-500 dark:text-slate-400">
            Import a job advertisement by pasting text or providing a URL. Sanity AI will extract and structure responsibilities and qualifications for matching.
          </p>
          ...
          >
            ...
            <span>Import Your First Job Description</span>
          </Link>
        </div>
      ) : filteredJds.length === 0 ? (
        <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center dark:border-slate-800 dark:bg-slate-900">
          <p className="text-sm text-slate-500 dark:text-slate-400">
            No job descriptions matched &ldquo;{searchQuery}&rdquo;.
          </p>
        </div>
      ) : (
        /* JDs Grid */
        <div data-testid="jds-grid" className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
          {filteredJds.map((jd) => (
            <JdCard key={jd._id} jd={jd} onRefresh={handleRefresh} />
          ))}
        </div>
      )}
    </div>
